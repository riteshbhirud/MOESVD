...
from .deepseek.modeling_deepseek import  DeepseekMLP, DeepseekAttention
...
class  SVD_DeepseekMLP(nn.Module):
    def __init__(self, config,  low_rank, shared_gate_v, shared_up_v, shared_down_v, hidden_size = None, intermediate_size = None, group_idx=None):
        super().__init__()
        self.config = config
        ...
        '''
        self.gate_proj_u = nn.Linear(low_rank, self.intermediate_size, bias=False)
        self.up_proj_u = nn.Linear(low_rank, self.intermediate_size, bias=False)
        self.down_proj_u = nn.Linear(low_rank, self.hidden_size, bias=False)

        self.shared_gate_v = shared_gate_v  # low_rank, self.intermediate_size
        self.shared_up_v = shared_up_v # low_rank self.intermediate_size
        self.shared_down_v = shared_down_v  # low_rank, self.hidden_size
    ...
    def forward(self, x):
        ...
        down_proj = self.down_proj_u(self.shared_down_v(self.act_fn(self.gate_proj_u(self.shared_gate_v(x))) * self.up_proj_u(self.shared_up_v(x))))
...
'''class SVD_DeepseekMoE(nn.Module):
    """
    ...
    def __init__(self, config, ratio=1):
        ...
        shared_gate_v = nn.Linear(self.hidden_dim, self.low_rank, bias=False)
        shared_up_v = nn.Linear(self.hidden_dim, self.low_rank, bias=False)
        shared_down_v = nn.Linear(self.ffn_dim, self.low_rank, bias=False)

        self.experts = nn.ModuleList([SVD_DeepseekMLP(config, low_rank=self.low_rank, intermediate_size = config.moe_intermediate_size, shared_gate_v= shared_gate_v ,shared_up_v= shared_up_v, shared_down_v=shared_down_v) for i in range(config.n_routed_experts)])
...
class SVD_DeepseekMoE(nn.Module):
    def __init__(self, config, ratio=1):
        ...
        self.total_experts = config.n_routed_experts  # 总专家数量，应该是64
        self.num_experts_shared_group = self.total_experts // self.num_groups  # 每组的专家数量

        # 创建共享参数的列表
        self.shared_gate_v_list = nn.ModuleList()
        self.shared_up_v_list = nn.ModuleList()
        self.shared_down_v_list = nn.ModuleList()

        for _ in range(self.num_groups):
            shared_gate_v = nn.Linear(self.hidden_dim, self.low_rank, bias=False)
            shared_up_v = nn.Linear(self.hidden_dim, self.low_rank, bias=False)
            shared_down_v = nn.Linear(self.ffn_dim, self.low_rank, bias=False)
            self.shared_gate_v_list.append(shared_gate_v)
            self.shared_up_v_list.append(shared_up_v)
            self.shared_down_v_list.append(shared_down_v)

        # 创建专家列表，并将专家分配到不同的组
        self.experts = nn.ModuleList()
        for group_idx in range(self.num_groups):
            shared_gate_v = self.shared_gate_v_list[group_idx]
            shared_up_v = self.shared_up_v_list[group_idx]
            shared_down_v = self.shared_down_v_list[group_idx]
            for _ in range(self.num_experts_shared_group):
                expert = SVD_DeepseekMLP(
                    config,
                    low_rank=self.low_rank,
                    intermediate_size=config.moe_intermediate_size,
                    shared_gate_v=shared_gate_v,
                    shared_up_v=shared_up_v,
                    shared_down_v=shared_down_v,
                    group_idx=group_idx
                )
...
class SVD_DeepseekAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: DeepseekConfig, layer_idx: Optional[int] = None, ratio=1):
        super().__init__()
        self.config = config
