...
class SVD_PhiMoESparseMoeBlock(nn.Module):
    def __init__(self, config: PhiMoEConfig, ratio=1):
        ...
        self.experts = nn.ModuleList([
            PhiMoECompressedExpert(config, self.ratio)
...
class PhiMoECompressedExpert(nn.Module):
    def __init__(self, config: PhiMoEConfig, ratio):
        super().__init__()
        self.hidden_dim = config.hidden_size
        self.ffn_dim = config.intermediate_size
        self.ratio = ratio
        ...
        self.w1_u = nn.Linear(low_rank, self.ffn_dim, bias=False)
        self.w1_v = nn.Linear(self.hidden_dim, low_rank, bias=False)
        self.w2_u = nn.Linear(low_rank, self.hidden_dim, bias=False)
        self.w2_v = nn.Linear(self.ffn_dim, low_rank, bias=False)
        self.w3_u = nn.Linear(low_rank, self.ffn_dim, bias=False)
        self.w3_v = nn.Linear(self.hidden_dim, low_rank, bias=False)
        
        self.act_fn = ACT2FN[config.hidden_act]

    def forward(self, hidden_states):
        w1_out = self.act_fn(self.w1_u(self.w1_v(hidden_states)))
        w3_out = self.w3_u(self.w3_v(hidden_states))
        current_hidden_states = w1_out * w3_out
        current_hidden_states = self.w2_u(self.w2_v(current_hidden_states))
